...
namespace aodv_trust
{
NS_OBJECT_ENSURE_REGISTERED(AodvTrustRoutingProtocol);

/// UDP Port for AODV control traffic
const uint32_t AodvTrustRoutingProtocol::AODV_PORT = 654;
...
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::aodv_trust::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Aodv")
...
//-----------------------------------------------------------------------------
AodvTrustRoutingProtocol::AodvTrustRoutingProtocol()
    : m_rreqRetries(2),
      m_ttlStart(1),
      m_ttlIncrement(2),
      m_ttlThreshold(7),
      ...
      /* [MODIFIKASI KELOMPOK] Default fitur trust aktif */
      m_enableTrust(true)
{
    m_nb.SetCallback(MakeCallback(&AodvTrustRoutingProtocol::SendRerrWhenBreaksLinkToNextHop, this));
    
    /* [MODIFIKASI KELOMPOK] Set nilai ambang batas kepercayaan default (0.4) */
    m_trustThreshold = 0.4; 
}

TypeId
AodvTrustRoutingProtocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::aodv_trust::AodvTrustRoutingProtocol")
            .SetParent<Ipv4RoutingProtocol>()
            .SetGroupName("Aodv")
            .AddConstructor<AodvTrustRoutingProtocol>()
            
            /* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
             * [MODIFIKASI KELOMPOK] Menambahkan Atribut 'EnableTrust' 
             * Agar bisa diatur ON/OFF melalui script simulasi tanpa re-compile
             * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
            .AddAttribute("EnableTrust",
                          "Enable or disable Trust Mechanism (True = Trust AODV, False = Standard AODV)",
                          BooleanValue(true),
                          MakeBooleanAccessor(&AodvTrustRoutingProtocol::m_enableTrust),
                          MakeBooleanChecker())
            /* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
            ...
            .AddAttribute("NodeTraversalTime",
                          ...
                          MakeTimeAccessor(&AodvTrustRoutingProtocol::m_nodeTraversalTime),
                          MakeTimeChecker())
            .AddAttribute(
                "NextHopWait",
                "Period of our waiting for the neighbour's RREP_ACK = 10 ms + NodeTraversalTime",
                TimeValue(MilliSeconds(50)),
                MakeTimeAccessor(&AodvTrustRoutingProtocol::m_nextHopWait),
                MakeTimeChecker())
            .AddAttribute("ActiveRouteTimeout",
                          "Period of time during which the route is considered to be valid",
                          TimeValue(Seconds(3)),
                          MakeTimeAccessor(&AodvTrustRoutingProtocol::m_activeRouteTimeout),
                          MakeTimeChecker())
            .AddAttribute("MyRouteTimeout",
                          ...
                          MakeTimeAccessor(&AodvTrustRoutingProtocol::m_myRouteTimeout),
                          MakeTimeChecker())
            .AddAttribute("BlackListTimeout",
                          "Time for which the node is put into the blacklist = RreqRetries * "
                          "NetTraversalTime",
                          TimeValue(Seconds(5.6)),
                          MakeTimeAccessor(&AodvTrustRoutingProtocol::m_blackListTimeout),
                          MakeTimeChecker())
            .AddAttribute("DeletePeriod",
                          ...
                          MakeTimeAccessor(&AodvTrustRoutingProtocol::m_deletePeriod),
                          MakeTimeChecker())
            .AddAttribute("NetDiameter",
                          "Net diameter measures the maximum possible number of hops between two "
                          "nodes in the network",
                          UintegerValue(35),
                          MakeUintegerAccessor(&AodvTrustRoutingProtocol::m_netDiameter),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute(
                "NetTraversalTime",
                "Estimate of the average net traversal time = 2 * NodeTraversalTime * NetDiameter",
                TimeValue(Seconds(2.8)),
                MakeTimeAccessor(&AodvTrustRoutingProtocol::m_netTraversalTime),
                MakeTimeChecker())
            .AddAttribute(
                "PathDiscoveryTime",
                "Estimate of maximum time needed to find route in network = 2 * NetTraversalTime",
                TimeValue(Seconds(5.6)),
                MakeTimeAccessor(&AodvTrustRoutingProtocol::m_pathDiscoveryTime),
                MakeTimeChecker())
            .AddAttribute("MaxQueueLen",
                          "Maximum number of packets that we allow a routing protocol to buffer.",
                          UintegerValue(64),
                          MakeUintegerAccessor(&AodvTrustRoutingProtocol::SetMaxQueueLen,
                                               &AodvTrustRoutingProtocol::GetMaxQueueLen),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("MaxQueueTime",
                          "Maximum time packets can be queued (in seconds)",
                          TimeValue(Seconds(30)),
                          MakeTimeAccessor(&AodvTrustRoutingProtocol::SetMaxQueueTime,
                                           &AodvTrustRoutingProtocol::GetMaxQueueTime),
                          MakeTimeChecker())
            .AddAttribute("AllowedHelloLoss",
                          "Number of hello messages which may be loss for valid link.",
                          UintegerValue(2),
                          MakeUintegerAccessor(&AodvTrustRoutingProtocol::m_allowedHelloLoss),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("GratuitousReply",
                          "Indicates whether a gratuitous RREP should be unicast to the node "
                          "originated route discovery.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&AodvTrustRoutingProtocol::SetGratuitousReplyFlag,
                                              &AodvTrustRoutingProtocol::GetGratuitousReplyFlag),
                          MakeBooleanChecker())
            .AddAttribute("DestinationOnly",
                          "Indicates only the destination may respond to this RREQ.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&AodvTrustRoutingProtocol::SetDestinationOnlyFlag,
                                              &AodvTrustRoutingProtocol::GetDestinationOnlyFlag),
                          MakeBooleanChecker())
            .AddAttribute("EnableHello",
                          "Indicates whether a hello messages enable.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&AodvTrustRoutingProtocol::SetHelloEnable,
                                              &AodvTrustRoutingProtocol::GetHelloEnable),
                          MakeBooleanChecker())
            .AddAttribute("EnableBroadcast",
                          "Indicates whether a broadcast data packets forwarding enable.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&AodvTrustRoutingProtocol::SetBroadcastEnable,
                                              &AodvTrustRoutingProtocol::GetBroadcastEnable),
                          MakeBooleanChecker())
            .AddAttribute("UniformRv",
                          "Access to the underlying UniformRandomVariable",
                          StringValue("ns3::UniformRandomVariable"),
                          MakePointerAccessor(&AodvTrustRoutingProtocol::m_uniformRandomVariable),
                          MakePointerChecker<UniformRandomVariable>());
    return tid;
}

void
AodvTrustRoutingProtocol::SetMaxQueueLen(uint32_t len)
{
    m_maxQueueLen = len;
...
void
AodvTrustRoutingProtocol::SetMaxQueueTime(Time t)
{
    m_maxQueueTime = t;
...
void
AodvTrustRoutingProtocol::DoDispose()
{
    m_ipv4 = nullptr;
...
void
AodvTrustRoutingProtocol::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
...
int64_t
AodvTrustRoutingProtocol::AssignStreams(int64_t stream)
{
...
void
AodvTrustRoutingProtocol::Start()
{
    ...
    m_rreqRateLimitTimer.SetFunction(&AodvTrustRoutingProtocol::RreqRateLimitTimerExpire, this);
    m_rreqRateLimitTimer.Schedule(Seconds(1));

    m_rerrRateLimitTimer.SetFunction(&AodvTrustRoutingProtocol::RerrRateLimitTimerExpire, this);
    m_rerrRateLimitTimer.Schedule(Seconds(1));
}

Ptr<Ipv4Route>
AodvTrustRoutingProtocol::RouteOutput(Ptr<Packet> p,
...
void
AodvTrustRoutingProtocol::DeferredRouteOutput(Ptr<const Packet> p,
                                     const Ipv4Header& header,
...
bool
AodvTrustRoutingProtocol::RouteInput(Ptr<const Packet> p,
                            const Ipv4Header& header,
                            ...
                            const ErrorCallback& ecb)
{
    ...
    /* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     * [MODIFIKASI UTAMA - LOGIKA KEAMANAN]
     * Sebelum memproses paket, cek reputasi pengirim (Origin).
     * 1. Cek apakah fitur Trust diaktifkan (m_enableTrust).
     * 2. Cek apakah Node Pengirim terpercaya (IsTrusted).
     * Jika tidak terpercaya, paket langsung DIBUANG (Drop).
     * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    if (m_enableTrust && !IsTrusted(origin))
    {
        NS_LOG_LOGIC("Security Alert: Packet from UNTRUSTED node " << origin << " dropped.");
        return false; // Drop paket, jangan diproses lebih lanjut
    }
    /* +++++++++++++++++++++ [AKHIR MODIFIKASI] ++++++++++++++++++++++++++++++ */
...
bool
AodvTrustRoutingProtocol::Forwarding(Ptr<const Packet> p,
                            const Ipv4Header& header,
                            UnicastForwardCallback ucb,
                            ErrorCallback ecb)
{
...
void
AodvTrustRoutingProtocol::SetIpv4(Ptr<Ipv4> ipv4)
{
    NS_ASSERT(ipv4);
    NS_ASSERT(!m_ipv4);
    ...
    Simulator::ScheduleNow(&AodvTrustRoutingProtocol::Start, this);
}

void
AodvTrustRoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&AodvTrustRoutingProtocol::RecvAodv, this));
    ...
    mac->TraceConnectWithoutContext("DroppedMpdu",
                                    MakeCallback(&AodvTrustRoutingProtocol::NotifyTxError, this));
}

void
AodvTrustRoutingProtocol::NotifyTxError(WifiMacDropReason reason, Ptr<const WifiMpdu> mpdu)
{
    m_nb.GetTxErrorCallback()(mpdu->GetHeader());
}

void
AodvTrustRoutingProtocol::NotifyInterfaceDown(uint32_t i)
{
    NS_LOG_FUNCTION(this << m_ipv4->GetAddress(i, 0).GetLocal());
...
void
AodvTrustRoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
...
void
AodvTrustRoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    NS_LOG_FUNCTION(this);
...
bool
AodvTrustRoutingProtocol::IsMyOwnAddress(Ipv4Address src)
{
...
Ptr<Ipv4Route>
AodvTrustRoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
...
void
AodvTrustRoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    if (m_rreqCount == m_rreqRateLimit)
    {
        Simulator::Schedule(m_rreqRateLimitTimer.GetDelayLeft() + MicroSeconds(100),
                            &AodvTrustRoutingProtocol::SendRequest,
                            this,
                            dst);
        return;
    }
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        Simulator::Schedule(Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10))),
                            &AodvTrustRoutingProtocol::SendTo,
                            this,
                            socket,
                            packet,
                            destination);
...
void
AodvTrustRoutingProtocol::SendTo(Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, AODV_PORT));
}

void
AodvTrustRoutingProtocol::ScheduleRreqRetry(Ipv4Address dst)
{
    NS_LOG_FUNCTION(this << dst);
    ...
    m_addressReqTimer[dst].SetFunction(&AodvTrustRoutingProtocol::RouteRequestTimerExpire, this);
...
void
AodvTrustRoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
...
bool
AodvTrustRoutingProtocol::UpdateRouteLifeTime(Ipv4Address addr, Time lifetime)
{
...
void
AodvTrustRoutingProtocol::UpdateRouteToNeighbor(Ipv4Address sender, Ipv4Address receiver)
{
...
void
AodvTrustRoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);
    RreqHeader rreqHeader;
...
void
AodvTrustRoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
...
void
AodvTrustRoutingProtocol::SendReplyByIntermediateNode(RoutingTableEntry& toDst,
                                             RoutingTableEntry& toOrigin,
                                             bool gratRep)
{
    NS_LOG_FUNCTION(this);
    ...
    if (toDst.GetHop() == 1)
    {
        ...
        toNextHop.m_ackTimer.SetFunction(&AodvTrustRoutingProtocol::AckTimerExpire, this);
...
void
AodvTrustRoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
...
void
AodvTrustRoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
...
void
AodvTrustRoutingProtocol::RecvReplyAck(Ipv4Address neighbor)
{
    NS_LOG_FUNCTION(this);
    RoutingTableEntry rt;
...
void
AodvTrustRoutingProtocol::ProcessHello(const RrepHeader& rrepHeader, Ipv4Address receiver)
{
...
void
AodvTrustRoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
...
void
AodvTrustRoutingProtocol::RouteRequestTimerExpire(Ipv4Address dst)
{
    NS_LOG_LOGIC(this);
    RoutingTableEntry toDst;
...
void
AodvTrustRoutingProtocol::HelloTimerExpire()
{
    NS_LOG_FUNCTION(this);
...
void
AodvTrustRoutingProtocol::RreqRateLimitTimerExpire()
{
    NS_LOG_FUNCTION(this);
    m_rreqCount = 0;
    m_rreqRateLimitTimer.Schedule(Seconds(1));
...
void
AodvTrustRoutingProtocol::RerrRateLimitTimerExpire()
{
    NS_LOG_FUNCTION(this);
    m_rerrCount = 0;
    m_rerrRateLimitTimer.Schedule(Seconds(1));
...
void
AodvTrustRoutingProtocol::AckTimerExpire(Ipv4Address neighbor, Time blacklistTimeout)
{
    NS_LOG_FUNCTION(this);
...
void
AodvTrustRoutingProtocol::SendHello()
{
    NS_LOG_FUNCTION(this);
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        Simulator::Schedule(jitter, &AodvTrustRoutingProtocol::SendTo, this, socket, packet, destination);
    }
}

void
AodvTrustRoutingProtocol::SendPacketFromQueue(Ipv4Address dst, Ptr<Ipv4Route> route)
{
    NS_LOG_FUNCTION(this);
...
void
AodvTrustRoutingProtocol::SendRerrWhenBreaksLinkToNextHop(Ipv4Address nextHop)
{
...
void
AodvTrustRoutingProtocol::SendRerrWhenNoRouteToForward(Ipv4Address dst,
                                              uint32_t dstSeqNo,
                                              Ipv4Address origin)
{
    NS_LOG_FUNCTION(this);
...
void
AodvTrustRoutingProtocol::SendRerrMessage(Ptr<Packet> packet, std::vector<Ipv4Address> precursors)
{
    NS_LOG_FUNCTION(this);
...
Ptr<Socket>
AodvTrustRoutingProtocol::FindSocketWithInterfaceAddress(Ipv4InterfaceAddress addr) const
{
...
Ptr<Socket>
AodvTrustRoutingProtocol::FindSubnetBroadcastSocketWithInterfaceAddress(Ipv4InterfaceAddress addr) const
{
    NS_LOG_FUNCTION(this << addr);
...
void
AodvTrustRoutingProtocol::DoInitialize()
{
    NS_LOG_FUNCTION(this);
    uint32_t startTime;
    if (m_enableHello)
    {
        m_htimer.SetFunction(&AodvTrustRoutingProtocol::HelloTimerExpire, this);
        startTime = m_uniformRandomVariable->GetInteger(0, 100);
        NS_LOG_DEBUG("Starting at time " << startTime << "ms");
        m_htimer.Schedule(MilliSeconds(startTime));
    }
    // MODIFIED: Fixed parent class call
    Ipv4RoutingProtocol::DoInitialize();
}

/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * [MODIFIKASI KELOMPOK] IMPLEMENTASI FUNGSI MANAJEMEN TRUST
 * Bagian ini berisi logika perhitungan nilai kepercayaan.
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */

void
AodvTrustRoutingProtocol::UpdateTrust(Ipv4Address node, double value)
{
    // Jika node baru, inisialisasi dengan nilai netral (0.5)
    if (m_trustTable.find(node) == m_trustTable.end())
    {
        m_trustTable[node] = 0.5; 
    }
    
    // Update nilai trust (Increment/Decrement)
    m_trustTable[node] += value;
    
    // Clamping: Pastikan nilai tetap di rentang 0.0 - 1.0
    if (m_trustTable[node] > 1.0) m_trustTable[node] = 1.0;
    if (m_trustTable[node] < 0.0) m_trustTable[node] = 0.0;
    
    NS_LOG_INFO("Trust System Update: Node " << node << " Current Trust Score=" << m_trustTable[node]);
}

bool
AodvTrustRoutingProtocol::IsTrusted(Ipv4Address node)
{
    // Kebijakan: Node yang belum dikenal dianggap 'Trusted' sementara (Optimistic approach)
    if (m_trustTable.find(node) == m_trustTable.end()) 
    {
        return true; 
    }
    
    // Keputusan: Trusted jika nilai >= Threshold (0.4)
    return m_trustTable[node] >= m_trustThreshold;
}

double
AodvTrustRoutingProtocol::GetTrust(Ipv4Address node)
{
    if (m_trustTable.find(node) == m_trustTable.end()) return 0.5;
