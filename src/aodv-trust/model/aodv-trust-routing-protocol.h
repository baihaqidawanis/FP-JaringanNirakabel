...
#ifndef AODV_TRUST_ROUTING_PROTOCOL_H
#define AODV_TRUST_ROUTING_PROTOCOL_H
 ...
 * \ingroup aodv
 *
 * \brief AODV Trust routing protocol (Modified)
 */
class AodvTrustRoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    /**
    ...
    /// constructor
    AodvTrustRoutingProtocol();
    ~AodvTrustRoutingProtocol() override;
...
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * [MODIFIKASI KELOMPOK - FITUR AODV TRUST]
 * Bagian ini ditambahkan untuk menyimpan nilai kepercayaan (Trust) 
 * dan parameter konfigurasi keamanan.
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    
    // Tabel untuk menyimpan nilai Trust tiap Node (Key: IP, Value: Score 0.0 - 1.0)
    std::map<Ipv4Address, double> m_trustTable;
    
    // Tabel untuk menyimpan Link Quality (Link Reliability)
    std::map<Ipv4Address, double> m_linkQualityTable;

    // Parameter Ambang Batas (Threshold). Node dengan nilai di bawah ini dianggap jahat.
    double m_trustThreshold; 

    // Saklar untuk mengaktifkan/menonaktifkan fitur Trust (Untuk skenario perbandingan)
    bool m_enableTrust; 
    
    // Fungsi Helper buatan sendiri untuk manajemen Trust
    void UpdateTrust(Ipv4Address node, double value);
    double GetTrust(Ipv4Address node);
    bool IsTrusted(Ipv4Address node);
...
#endif /* AODV_TRUST_ROUTING_PROTOCOL_H */